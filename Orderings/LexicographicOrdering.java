 ...
 */
public class LexicographicOrdering<A, B> extends Ordering<Pair<A, B>> {
    private final BiPredicate<A, A> r1;
    private final BiPredicate<B, B> r2;

    /**
     * Define a lexicographic ordering with two bi-predicates for r1 and r2.
     */
    public LexicographicOrdering(BiPredicate<A, A> r1, BiPredicate<B, B> r2) {
        super((p, q) -> {
            A x1 = p.a();
            B y1 = p.b();
            A x2 = q.a();
            ...
            return r1.test(x1, x2) || (x1.equals(x2) && r2.test(y1, y2));
        });
